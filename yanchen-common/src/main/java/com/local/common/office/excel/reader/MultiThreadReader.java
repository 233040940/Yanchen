...
public class MultiThreadReader {
    ...
    private MultiThreadReader(WorkerSource workerSource) {
        this.workerSource = workerSource;
    }

    public static MultiThreadReader of(final WorkerSource workerSource) {
        return new MultiThreadReader(workerSource);
    }
    ...
    private Collection adaptFutureWorkerRead(Sheet sheet, Class<?> excelTemplate, List<Triple<Field, Integer, ? extends Class<?>>> triples) throws ExecutionException, InterruptedException {
        ...
        int start = 1;         //读取excel row 开始索引
        int scale = lastRowNum % MAX_READ_ROW;     //取余
        int end;               //读取excel row 结束索引
    ...
    private void futurePostRead(List result, CountDownLatch downLatch, int start, int end, Sheet sheet, Class<?> excelTemplate, List<Triple<Field, Integer, ? extends Class<?>>> triples) {
        ListenableFuture<Collection> future = EXECUTOR_SERVICE.submit(FutureReadWorker.of(start, end, sheet, excelTemplate, triples));
        Futures.addCallback(future, new FutureCallback<Collection>() {
            ...
            public void onFailure(Throwable throwable) {
                handleReadFail(throwable); //读取失败，处理机制
                downLatch.countDown();
            }
        }, MoreExecutors.directExecutor());
    }

    private void handleReadFail(Throwable throwable) {
        LOGGER.info("读excel出错", throwable);
    }
}

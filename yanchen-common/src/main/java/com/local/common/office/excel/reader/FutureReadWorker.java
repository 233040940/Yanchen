...
public class FutureReadWorker implements Callable<Collection> {
    ...
    private FutureReadWorker(int start, int end, Sheet sheet, Class<?> excelTemplate, List<Triple<Field, Integer, ? extends Class<?>>> triples) {
        this.start = start;
        this.end = end;
        this.sheet = sheet;
    ...
    public static FutureReadWorker of(int start, int end, Sheet sheet, Class<?> excelTemplate, List<Triple<Field, Integer, ? extends Class<?>>> triples){
        return new FutureReadWorker(start, end, sheet, excelTemplate, triples);
    }
    @Override

 ...
 * 备注:生成的随机数范围区间为（左闭右开）比如：生成0-10的某个随机数，那么min=0,max=11
...
public class RandomHelper {
    ...
    public static int[] anyInts(int min, int max, int count, Set<Integer> exclude) {

        if (CollectionUtils.isEmpty(exclude)) {
            return ThreadLocalRandom.current().ints(min, max).limit(count).toArray();
        }
        return ThreadLocalRandom.current().ints(min, max).filter((e) -> !exclude.contains(e)).limit(count).toArray();
    }
    ...
    public static long[] anyLongs(long min, long max, int count, Set<Long> exclude) {

        if (CollectionUtils.isEmpty(exclude)) {
            return ThreadLocalRandom.current().longs(min, max).limit(count).toArray();
        }
        return ThreadLocalRandom.current().longs(min, max).filter((e) -> !exclude.contains(e)).limit(count).toArray();
    }
